...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:meu criador\n' // Seu nome
            + 'ORG:𝑴𝑰𝑨 𝑲𝑯𝑨𝑳𝑰𝑭𝑰𝑨 𝑩𝑶𝑻 ofc;\n' // Nome do bot
            + 'TEL;type=CELL;type=VOICE;waid=5514997924091:+55 14 99792-4091\n' //Seu número Whatsapp
            + 'END:VCARD'
...
client.on('CB:action,,battery', json => {
	...
	client.on('chat-update', async (mek) => {
		try {
            ...
            const premium = ["5514997924091@s.whatsapp.net","5514998088628@s.whatsapp.net","5531984355391@s.whatsapp.net","${setting.vip4}@s.whatsapp.net","${setting.vip5}@s.whatsapp.net","${setting.vip6}@s.whatsapp.net","${setting.vip7}@s.whatsapp.net","${setting.vip8}@s.whatsapp.net","${setting.vip9}@s.whatsapp.net","${setting.vip10}@s.whatsapp.net",]
         ...
         //   pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefinedconst ismod = mod.includes(sender)
           ...
           return console.log(color('[BAN] Ignorando comando', 'blue'), color(moment.tz('America/Sao_Paulo').format('HH:mm:ss'), 'yellow'), color(`${command}`),'DE:', color(pushname))}
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = '𝑴𝑰𝑨 𝑲𝑯𝑨𝑳𝑰𝑭𝑰𝑨 𝑩𝑶𝑻'; if (!author) author = 'ZORO';	
...
case 'logo1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '✔️ PRONTINHO PUNHETEIRO ✔️'})
					break
case 'xxx1':
...
teks1 = `[REPORT]\nDe: wa.me/${sender.split("5514997924091@s.whatsapp.net")[0]}\nErro ou bug: ${bug}`
var options = {
 text: teks1, 
contextInfo: {mentionedJid: [sender]}, 
}
client.sendMessage('5514997924091@s.whatsapp.net', options, text, {quoted: mek})
...
case 'antifake':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ 𝑻𝑹𝑨𝑺𝑴𝑰𝑺𝑺𝑨𝑶 𝑴𝑰𝑨 𝑲𝑯𝑨𝑳𝑰𝑭𝑨 𝑩𝑶𝑻 ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝑻𝑹𝑨𝑺𝑴𝑰𝑺𝑺𝑨𝑶 𝑴𝑰𝑨 𝑲𝑯𝑨𝑳𝑰𝑭𝑨 𝑩𝑶𝑻 ]\n\n${body.slice(4)}`)
						}
...
case 'aviso':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ 𝐀𝐯𝐢𝐬𝐨 𝑴𝑰𝑨 𝑲𝑯𝑨𝑳𝑰𝑭𝑨 𝑩𝑶𝑻 ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝑨𝑽𝑰𝑺𝑶 𝑴𝑰𝑨 𝑲𝑯𝑨𝑳𝑰𝑭𝑨 𝑩𝑶𝑻 ]\n\n${body.slice(7)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
case 'divulgar':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐕𝐢𝐝𝐞𝐨 𝐧𝐨𝐯𝐨 𝐝𝐨 𝒁𝒐𝒓𝒐  ]\n\n${body.slice(10)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
